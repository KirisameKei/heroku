...
async def on_member_join(member):#新規の人が来たら反応
    ...
    if server == 585998962050203672:#もしけいの実験サーバなら
        #flag = False
        #async for UserRoleLog in RoleLogChannel.history():
            #HadRole = await RoleLogChannel.fetch_message(UserRoleLog.id)
            #if UserRoleLog.content.startswith(comeid):
                #Userid = UserRoleLog.content[0:19]
                #HatsugenKinshi = f"{UserRoleLog.content}".replace(UserId,"")
                #AddRole = discord.utils.get(member.guild.roles,id = HatsugenKinshi)
