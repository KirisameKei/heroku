...
async def on_guild_channel_create(channel):
    if channel.guild.id == 624551872933527553:
        ...
        await channel.send(f"{me.mention}\n新しくチャンネルが作成されました。コードの追加お願いします。")
        await channel.send(f"{reiwa.mention}\n新しくチャンネルが作成されました。ログ鯖に同じ名前のチャンネルを作成してください。\
            (いつか自動で作れるようにしたいです。それまでメンション行きます、ごめんなさい)")
            ...
            chan == 597122356606926870 or chan == 586420890678591509:
                ...
                if reg_res:
                    ...
                    if reg_res.group(1) in citycodes.keys():
                        ...
                        msg += "です。\n"
                        KyouMaxTemp = resp['forecasts'][0]['temperature']['max']['celsius']
