...
async def login_bonus(message,client1,m):
    ...
    if message.content == "利子を付与します" or message.content == "/mypt" or message.content.startswith("/otherpt ") or \
        message.content.startswith("/addpt ") or message.content.startswith("/usept "):
...
もしよろしければ、<#586571234276540449>もお願いします。")
    ...
    if message.content == "/omikuji" or message.content == "/speca" or message.content == "/meigen" or \
        ...
        message.content.startswith("/role_count "):
        if not message.channel.id in channel_dic.my_guild_allow_command_channel:
            await m("ここで実行しないでください！")
            return
...
明日の最高気温は"+AsuMaxTemp+"℃で、最低気温は"+AsuMinTemp+"℃です。")
        ...
        if message.content.startswith("/stimer "):
            swait = message.content.replace("/stimer ","")
            p = re.compile(r"^[0-9 ]+$")
            if not p.match(swait):
                await m("時間は半角数字で指定してください。")
                return
        ...
        if message.content.startswith("/mtimer "):
            mwait = message.content.replace("/mtimer ","")
            p = re.compile(r"^[0-9. ]+$")
            if not p.match(mwait):
                await m("時間は半角数字で指定してください。")
                return
            kesu = mwait.split()[0]#str     "3 "
            jikan = kesu.replace(" ","")#str,数字のみ
            kazu = float(jikan)
            if not kazu >= 1:
                fixed = jikan.replace("-","")
        ...
        if message.content.startswith("/htimer "):
            hwait = message.content.replace("/htimer ","")
            p = re.compile(r"^[0-9. ]+$")
            if not p.match(hwait):
                await m("時間は半角数字で指定してください。")
                return
            kesu = hwait.split()[0]#str     "3 "
            jikan = kesu.replace(" ","")#str,数字のみ
            kazu = float(jikan)
            if not kazu >= 1:
                fixed = jikan.replace("-","")
