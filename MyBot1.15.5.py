...
その他の機能\n\
```\n\
・「おはよう」、「こんにちは」、「こんばんは」の入った文字列を送ると返事をします。時間によって反応が変わります。\n\
・一週間に一度しりとりチャンネルをリセットして最初のワードを言います。(最近動いてないです。原因究明中)\n\
...
async def loop():
    ...
    okuchannel1 = client.get_channel()
    okuchannel2 = client.get_channel()
