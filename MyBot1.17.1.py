                    ...
                    if message.content == "/hide me":
                        ...
                        if p.fullmatch(msg):
                            ...
                            if discord.utils.get(hide.roles,name="かくれんぼなう"):
                                ...
                                if kazu != 598348241024581655 and kazu != 594396830071259154 and kazu != 594052349140402179:
                                    role = discord.utils.get(message.guild.roles,name = "かくれんぼなう")
                                    await hide.add_roles(role)
                                    await m(hide.name+"が隠れました。もーいーよ")
                                await m("botが隠れてもいいのかい？この鯖が動かなくなるよ？")
                        else:
...
/osusume movie      お勧めの動画を紹介します。紹(ry(めんどくさくなったので実装中止)\n\
/help               これを出します```\n\
その他の機能\n\
```\n\
...
async def loop():
    ...
    if now.month == 10 and now.day == 6 and now.hour == 21 and now.minute == 00:
        oubo = ["2レジ","akahase","ASSAULT_256","chorocra","Lucky_jp","okuma_256_qp","Ryu","takokokko","いろは(ranzamu)","ちゃん",\
            "れいわちゃん","世紀の科学者　うるあ","塩","蜜柑星p"]#応募者(敬称略)
