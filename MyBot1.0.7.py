...
async def on_guild_channel_create(channel):
    if channel.guild.id == 624551872933527553:
        ...
        reiwa = client.get_user(538977672391294986)
        channel = client.get_channel(624551872933527555)
        server = client.get_guild(633328124968435712)
        await server.create_text_channel(name = channel.name)
        await channel.send(f"{me.mention}\n新しいチャンネル「"+channel.name+"」が作成されました。コードの追加お願いします。")
        await channel.send(f"{reiwa.mention}\n新しくチャンネルが作成されました。ログ鯖に同じ名前のチャンネルを作成しました。適切なカテゴリへの振り分けをお願い致します。\
            (いつか自動で振り分けられるようにしたいです。それまでメンション行きます、ごめんなさい)")

@client.event
async def on_guild_channel_update(before,after):
    SyobatuCh = client.get_all_channels
    if before.name != after.name:
        me = client.get_user(523303776120209408)#けいのID
        channel = client.get_channel(636359382359080961)
        await channel.send(f"{me.mention}\nチャンネル名が変更されました。それがしょ鯖ならログ鯖の更新をお願いします")
...
async def on_message(message):#メッセージを受け取る
    ...
    if client.user != message.author:#送り主が自分の場合反応しない
        ...
        if message.channel.id == 635778829083869184:#dm画像
            channel = client.get_channel(636809744283074570)
            await channel.send(embed = embed)
        ...
        if message.channel.id == 630196403666288660:#ソビエトの巫女
            ...
            await channel.send(embed = embed)
        if message.channel.id == 636569854681284629:#猫咲夜
        ...
        if message.channel.id == 631792683538710549:#ryuuku07
            ...
            await channel.send(embed = embed)
        if message.channel.id == 633942827247599617:#マリネ
            channel = client.get_channel(633943017283387392)
            await channel.send(embed = embed)
        if message.channel.id == 635320398237663232:#はるさめ
        ...
        if message.guild.id == 585998962050203672:
            if message.author.id == 623229676860604422:#のりはまなら
                NorihamaMessageLogChannel = client.get_channel(634178723859398679)#チャンネルid
                await NorihamaMessageLogChannel.send(message.channel.name+"("+now+")\n"+message.content)

                if message.channel.name != "自己紹介":
                    if "ドイツ" in message.content or "こっち" in message.content:
                        await message.channel.purge(limit = 1)
        ...
        if message.channel.id == 634602609017225225 and message.author != client.user:
            ...
            if Touraku == "おめでとう！":
                if message.author.id != 623229676860604422:#のりはまでなければ
                    ...
                    GivePoint = random.randint(1,16)#与えるpt(pt)int
                    flag = False
                    PointLogChannel = client.get_channel(634602916233216020)
                    HitoyouChannel = client.get_channel(634604172808814593)
                    async for msg in PointLogChannel.history():
