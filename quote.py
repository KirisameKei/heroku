...
async def expand(message,client1,client4):
    embed = await fetch_embed(message,client1,client4)
    channel = client1.get_channel(message.channel.id)
    await channel.send(embed=embed)


async def fetch_embed(message,client1,client4):
    guild_id = int(message.content[32:50])
    if guild_id == 626739606674735134:
