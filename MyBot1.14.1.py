...
"+msg1+","+msg2+","+msg3+"を読んで、"+msg4+"をお願いします！")#歓迎メッセージ
...
async def on_message(message):#メッセージを受け取る
    ...
    if client.user != message.author:#送り主が自分の場合反応しない
        ...
        if "おはよう" in message.content:#メッセージの中に「おはよう」が入っていて
            if now >= "5:00" and now <= "11:00":#指定時間内なら
        ...
        if "こんばんは" in message.content:
            if now >= now >= "0:00" and now <= "6:00" or now >= "17:00" and now <= "23:59":
        ...
        if message.content.startswith("/"):
            ...
            chan == 597122356606926870:
                ...
                if message.content == "/meigen":
                    ...
                    "動くと撃つ！間違えた。撃つと動くだ。今すぐ動く","２時間前に出直してきな"]
                ...
                if message.content == "/accept":
                    if discord.utils.get(message.author.roles,name="新規"):
                        role = discord.utils.get(message.guild.roles,name = "新規")
                ...
                if message.content == "/osusume eshi":
                    ...
                    if choice == "p":
                        await m("https://twitter.com/papiermesser \nふみ切さん\n短編漫画家。とにかくストーリーがいい。たった一ページの漫画でうるっとくるものまである。他者への思いやりあふれる作品を量産する。過去に7年間連続で毎日投稿をしていたらしい。すごい。")
                    if choice == "q":
...
/osusume youtuber   お勧めのYoutuberを紹介します。紹介文は(ry(未実装)\n\
/osusume movie      お勧めの動画を紹介します。紹(ry(未実装)\n\
/help               これを出します```\n\
その他の機能\n\
```\n\
・「おはよう」、「こんにちは」、「こんばんは」の入った文字列を送ると返事をします。時間によって反応が変わります(が調子が悪いです)。\n\
