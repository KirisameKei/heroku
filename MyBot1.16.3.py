...
async def on_member_join(member):#新規の人が来たら反応
    ...
    if muhouid != 587909823665012757:
        if comeid == 607459082873602059:
...
async def on_message(message):#メッセージを受け取る
    ...
    if okch != 586002750823858186 and okca != 595072154068975625 and okca != 592579516858236928 and\
        okca !=598864281626476544 and okgu != 587909823665012757:
        ...
        if message.channel.id == 586420890678591509:
            if message.content == "/cancel":
                if discord.utils.get(message.author.roles, name = "応募中"):
                    ...
                    await message.channel.send(message.author.name+"さんがキャンセルしました")
                    await message.author.remove_roles(role)
                else:
                    await message.channel.send("まだ応募していません")
            if message.content == "/join":
                if discord.utils.get(message.author.roles, name = "応募中"):
                    await message.channel.send("既に参加済みです")
                else:    
                    role = discord.utils.get(message.guild.roles, name = "応募中")
                    await message.channel.send(message.author.name+"さんが酸化しました")
                    await message.author.add_roles(role)
...
もしよろしければ"+msg4+"もお願いします。")
                ...
                if message.content == "/osusume eshi":
                    esi = ["a","b","c","d","e","f","g","h","i","j","k","l","m","n","o","p","q"]
                    ...
                    if choice == "g":
                        await m("https://twitter.com/LEXUS_6737 \n虚無僧さん\n面白い漫画を描く人。~~ここ数か月更新が途絶えているので心配である。~~先ほど投稿がありました。生存しています。")
                    if choice == "h":
                    ...
                    if choice == "k":
                        await m("https://twitter.com/ayn398 \nあやのさん\n一枚漫画を描く。そこそこうまいし面白い。最近更新が途絶えているがプロフに育児中と書いてあるので多分それが原因だろう")
                    if choice == "l":
                ...
                #オススメ動画紹介
                if message.content == "/osusume movie":
                    movie = ["youtube"]
                    choice = (random.choice(movie))#紫戦、幽々子戦、こいし戦、
                    if choice == "a":
                        await m()
...
/osusume youtuber   お勧めのYoutuberを紹介します。紹介文は(ry(二人分だけ実装しました。まだ追加します)\n\
/osusume movie      お勧めの動画を紹介します。紹(ry(未実装)\n\
/help               これを出します```\n\
その他の機能\n\
```\n\
...
async def loop():
    ...
    now = datetime.now()
    channel = client.get_channel(586420858512343050)#お知らせ
    subchannel = client.get_channel(586420890678591509)#応募フォーム
    if now.month == 10 and now.day == 2 and now.hour == 21 and now.minute == 00:
        await channel.send("応募締め切りまであと3日です・・・")
    
    if now.month == 10 and now.day == 4 and now.hour == 21 and now.minute == 00:
        everyone = f"{member.mention}"
        await channel.send("応募締め切りまであと1日です・・・")
    
    if now.month == 10 and now.day == 5 and now.hour == 21 and now.minute == 00:
        await channel.send("締め切ります。以降の応募、キャンセルはできません。")
        await subchannel.send("締め切りました。このメッセージ以降の応募、キャンセルは認めません。")

    if now.month == 10 and now.day == 6 and now.hour == 20 and now.minute == 00:
        await channel.send("発表まであと1時間です・・・")

    if now.month == 10 and now.day == 6 and now.hour == 20 and now.minute == 50:
        await channel.send("発表まであと10分です・・・")
    
    if now.month == 10 and now.day == 6 and now.hour == 20 and now.minute == 59:
        await channel.send("発表まであと1分です・・・")
    if now.month == 10 and now.day == 6 and now.hour == 21 and now.minute == 00:
        oubo = ["akahase","ASSAULT_256","chorocra","Daikon","Lucky_jp","ramenOcean3Zk","Ryu_256","takokokko","いろは(ranzamu)",\
            "れいわちゃん","世紀の科学者　うるあ"]#応募者(敬称略)
        tousen1 = (random.choice(oubo))
        tousen2 = (random.choice(oubo))
        tousen3 = (random.choice(oubo))
        tousen4 = (random.choice(oubo))
        await channel.send("一等は__"+tousen1+"__さんです！おめでとうございます！")
        if tousen1 == tousen2:
            if tousen2 == tousen3:
                if tousen3 == tousen4:
                    await channel.send("二等も__"+tousen4+"__さんです！おめでとうございます！")
                    await channel.send("もう知らん！あなたはそうとう運がいい人だ！ガイア二本上げるよ！")
                else:
                    await channel.send("二等は__"+tousen4+"さんです！おめでとうございます！")
            else:
                await channel.send("二等は__"+tousen3+"__さんです！おめでとうございます！")
        else:
            await channel.send("二等は__"+tousen2+"__さんです！おめでとうございます！")
        await channel.send("当選者様にお願いです。転売はご遠慮ください。")
        await channel.send("惜しくも落選となってしまった皆様、まだオークションがあります。希望を捨てないでください。")
