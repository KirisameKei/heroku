...
now = datetime.datetime.now().strftime("%H:%M")#現在の時刻
...
async def on_member_join(member):#新規の人が来たら反応
    ...
    channel5 = client.get_channel(640833025822949387)
...
async def on_member_remove(member):#ユーザーがサーバを抜けたら反応
    ...
    if server == 585998962050203672:
        ...
        async for msg in mcidlogchannel.history():
            ...
            if UserIdMcid.content.startswith(str(member.id)):
                await UserIdMcid.delete()
...
async def on_message(message):#メッセージを受け取る
    ...
    if not message.author.bot:
        if message.guild.id == 585998962050203672:
            NikkanHatugensuuLogChannel = client.get_channel(641511982805024768)
            today = datetime.date.today()
            flag = False
            async for msg in NikkanHatugensuuLogChannel.history(limit = 1):
                TodayHatugensuu = await NikkanHatugensuuLogChannel.fetch_message(msg.id)
                if TodayHatugensuu.content.startswith(str(today)):
                    Hatugensuu = f"{TodayHatugensuu.content}".replace(str(today),"")
                    Hatugensuu = int(Hatugensuu) + 1
                    await NikkanHatugensuuLogChannel.send(str(today)+" "+str(Hatugensuu))
                    try:
                        await TodayHatugensuu.delete()
                        flag = True
                        break
    ...
    if message.author != client.user:
        if message.channel == message.author.dm_channel:
            channel = client.get_channel(639830406270681099)
            await channel.send(">>> "+message.author.name+"\n"+str(message.author.id)+"\n"+message.content)

        if message.channel.id == 639830406270681099:
            userid = message.content[0:18]
            p = re.compile( r'^[0-9]+$' )
            if p.fullmatch(userid):
                member = client.get_user(int(userid))
                content = f"{message.content}".replace(userid,"")
    ...
    #宣伝禁止
        ...
        if message.channel.id == 640833025822949387:
            ...
            if p.fullmatch(message.content):
                mcidlogchannel = client.get_channel(638912957421453322)
                mcidlist = message.content
                mcid = mcidlist.split(" ")
                Account = message.content.count(" ") + 1#半角スペースの数+1=アカウント数(int)
                for i in range(Account):
                    ...
                    async for msg in mcidlogchannel.history():
                        ...
                        if send == Mcid:
                            await message.channel.send(send+"はすでに登録されています。これが本当に現在の自分のMCIDならけいにお知らせください")
                            mcid[i] = ""
                            flag = True
                    if not flag:
                        await mcidlogchannel.send(str(message.author.id)+" "+send)
                if not flag:
                    ...
                    await message.channel.send("MCIDの報告が完了しました。\n報告済みのID>>>\n"+str(mcid))
            else:
        ...
        if message.channel.id == 640833084782018580:
            if "→" in message.content:
                ...
                if p.fullmatch(message.content):
                    ...
                    flag = False
                    async for msg in mcidlogchannel.history():
                        OldUserIdMcid = await mcidlogchannel.fetch_message(msg.id)#変更前のmcid(userid mcid)str
                        if OldUserIdMcid.content.startswith(str(message.author.id)):
                            mem_id = OldUserIdMcid.content[0:19]
                            OldMcid = f"{OldUserIdMcid.content}".replace(mem_id,"")#変更前のmcid(mcid)str
                            OldMcid_UserSay = message.content.split("→")[0]
                            if OldMcid == OldMcid_UserSay:
                                NewMcid = f"{message.content}".replace(OldMcid+"→","")
                                flag = False
                                async for msg in mcidlogchannel.history():
                                    UserIdMcid = await mcidlogchannel.fetch_message(msg.id)
                                    user_id = UserIdMcid.content[0:19]
                                    Mcid = f"{UserIdMcid.content}".replace(user_id,"")
                                    if NewMcid == Mcid:
                                        await message.channel.send(NewMcid+"はすでに登録されています。これが本当に現在の自分のMCIDならけいにお知らせください")
                                        flag = True
                                    if not flag:
                                        await mcidlogchannel.send(str(message.author.id)+" "+NewMcid)
                                        await message.channel.send("MCIDの変更を確認しました>>>\n"+OldMcid+"→"+NewMcid)
                                        await OldUserIdMcid.delete()
                                        flag = True
                                        break
                    if not flag:
                        await message.channel.send("まだ報告されていないMCIDです")
                else:
                    await message.channel.send("MCID変更報告に使用できない文字が含まれています")
            else:
                await message.channel.send("報告の形式は\n旧MCID→新MCIDです")

        if message.content.startswith("/mcid "):
            user_id = message.content[6:24]#userid(str)
            p = re.compile(r'^[0-9]+$')
            if p.fullmatch(user_id):
                ...
                async for msg in mcidlogchannel.history():
                    UserIdMcid = await mcidlogchannel.fetch_message(msg.id)
                    if UserIdMcid.content.startswith(user_id):
                        username = message.guild.get_member(int(user_id))
                        KirokuId = UserIdMcid.content[0:19]
                        Mcid = UserIdMcid.content.replace(KirokuId,"")
                        await message.channel.send(username.name+"さんのMCID:"+Mcid)
                        Hoyuusuu = Hoyuusuu + 1
                        flag = True
                if not flag:
                    await message.channel.send("まだMCIDを報告していないユーザーです")
                if flag:
                    await message.channel.send("以上"+str(Hoyuusuu)+"アカ")
            else:
...
/pt 他人のID        他人の保有ptを見ることができます。\n\
/mcid 他人のID      他人の報告しているMCIDを知れます。\n\
...
タイマー機能を使ったことによるいかなる被害もけいは責任を取りません\n\
    (要はこの機能に任せっきりで自分の用事を忘れるなよってこと。再起動したらリセットされます)\n\
/help               これを出します```\n\
")
                    await m("\
その他の機能(2/2)\n\
```\n\
・「ありがとう」の入った文字列を送ると返事をします。\n\
...
・指定チャンネル以外でディスコの招待リンクを貼ると消されて怒られます。(現在機能停止中)\n\
...
その縄は遥か昔から\n不浄な者の出入りを禁じるために使われてきたのよ"
    ...
    if not message.author.bot:
        if message.guild.id == 585998962050203672:
            HatugensuuLogChannel = client.get_channel(641264504696602656)
            LevelUpChannel = client.get_channel(641274239110086666)
            flag = False
            async for msg in HatugensuuLogChannel.history():
                UseridHatugensuu = await HatugensuuLogChannel.fetch_message(msg.id)
                user_id = UseridHatugensuu.content[0:18]
                Hatugensuu = f"{UseridHatugensuu.content}".replace(user_id+" ","")
                if user_id == str(message.author.id):
                    Hatugensuu = int(Hatugensuu) + 1
                    await HatugensuuLogChannel.send(user_id+" "+str(Hatugensuu))
                    try:
                        await UseridHatugensuu.delete()
                        if math.sqrt(Hatugensuu) % 1 == 0:
                            q = await LevelUpChannel.send(message.author.mention+"\nﾑﾑｯwwwwwwwﾚﾍﾞﾙｱｯﾌ゜wwwwwww【Lv"+str(math.floor(math.sqrt(Hatugensuu))-1)+"→Lv"+str(math.floor(math.sqrt(Hatugensuu)))+"】")
                            await q.add_reaction('\U0001F595')
                        flag = True
                        break
                    except discord.errors.NotFound:
                        await message.channel.send(f"{message.author.mention}連投のしすぎです！気をつけてください！")
            if not flag:
                await HatugensuuLogChannel.send(str(message.author.id)+" 1")
                q = await LevelUpChannel.send(message.author.mention+"\nﾑﾑｯwwwwwwwﾚﾍﾞﾙｱｯﾌ゜wwwwwww【Lv1】")
                await q.add_reaction('\U0001F595')

    #一周年企画用
    if not message.author.bot:
        if message.channel.id == 586420890678591509:
            if message.content == "/cancel":
                if discord.utils.get(message.author.roles,name="応募中"):
                    SankaLogChannel = client.get_channel(641173561926746114)
                    async for msg_3 in SankaLogChannel.history(limit = 1):
                        SankasyaItiran = await SankaLogChannel.fetch_message(msg_3.id)
                        mcidlogchannel = client.get_channel(638912957421453322)
                        async for msg_4 in mcidlogchannel.history():
                            UseridMcid = await mcidlogchannel.fetch_message(msg_4.id)
                            userid = UseridMcid.content[0:19]
                            if userid.startswith(str(message.author.id)):
                                Sanakasya = SankasyaItiran.content.split(" ")
                                Ninzuu = len(Sanakasya)
                                for i in range(Ninzuu):
                                    Mcid = UseridMcid.content[19:]
                                    if Sanakasya[i] == Mcid:
                                        if Ninzuu != 1:
                                            del Sanakasya[i]
                                            await SankaLogChannel.send(" ".join(Sanakasya))
    ...
    if message.author == client.user:
        ...
        if message.content == "日付変更をお知らせします。今日の日付："+str(Hiduke.year)+"年"+str(Hiduke.month)+"月"+str(Hiduke.day)+"日":
            truetoday = datetime.date.today()
            today = truetoday - datetime.timedelta(days = 1)
            yesterday = truetoday - datetime.timedelta(days = 2)
            NikkanHatugensuuLogChannel = client.get_channel(641511982805024768)
            async for msg in NikkanHatugensuuLogChannel.history(limit = 2):
            ...
            if HatugensuuNoSa > 0:
                ...
                HatugensuuNoSa = str(HatugensuuNoSa)
            await message.channel.send("昨日の発言数："+TodayHatugensuu+"\n前日比："+HatugensuuNoSa)
            await NikkanHatugensuuLogChannel.send(str(today)+" 0")


@tasks.loop(seconds = 60)#60秒に一回ループ
async def loop():
    now = datetime.datetime.now().strftime("%H:%M")
    weekday = datetime.datetime.today().weekday()#現在の曜日
    if weekday == 6:
        now = datetime.datetime.now().strftime("%H:%M")#現在の時刻
        channel = client.get_channel(603832801036468244)
        if now == "02:20":
